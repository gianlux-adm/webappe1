...
(function ($) {
    ...
    $.fn.columnFilter = function (options,completeCallback) {
        ...
        function fnCreateLabelRangeInput(oTable) {
            ...
            */
	        ...
	        function (oSettings, aData, iDataIndex) {
	            ...
	            var tmpValue = parseInt($(aData[_fnColumnIndex(index)]).html());
	            var iValue = tmpValue == "-" ? 0 : tmpValue * 1;
	            if (iMin == "" && iMax == "") {
	                return true;
	            }
        ...
        function fnCreateDateRangeInput(oTable, isRange) {

            var lblDa = "From";
            var lblTo = "To";
            if (isRange) {
                th.append("<span class='lbl' style='float:left;'>" + lblDa + "</span>");
            }
            var sFromId = oTable.attr("id") + '_range_from_' + i;
            var fromContainer = $("<div class='input-group' style='width:150px;float:left;'/>");
            th.append(fromContainer);
            var from = $('<input type="text" class="date_range_filter form-control" id="' + sFromId + '" rel="' + i + '"/>');

            var currentLocale = $("[data-lang]").attr("data-lang");
            ...
            fromContainer.append("<span class='input-group-addon'><span class='glyphicon glyphicon-calendar'></span></span>");

            if (isRange) {
                th.append("<span class='lbl' style='margin-left:10px;'>" + lblTo + "</span>");
            }
            var sToId = oTable.attr("id") + '_range_to_' + i;
            if (isRange) {
                var toContainer = $("<div class='input-group' style='width:150px;float:left;'/>");
                var to = $('<input type="text" class="date_range_filter form-control" id="' + sToId + '" rel="' + i + '"/>');
                toContainer.append(to);
                toContainer.append("<span class='input-group-addon'><span class='glyphicon glyphicon-calendar'></span></span>");
                th.append(toContainer);
                th.append("<span class='lbl' style='margin-left:10px;'>" + "" + "</span>");
	      ...
	      function (oSettings, aData, iDataIndex) {
	          ...
	          var dStartDate = from.data("DateTimePicker").date();

	          var dEndDate = from.data("DateTimePicker").date();

	          if (isRange) {
	              dEndDate = to.data("DateTimePicker").date();
	          }
	          ...
	          try {
	              if (aData[_fnColumnIndex(index)] == null || aData[_fnColumnIndex(index)] == "")
	                  return false;
	              dCellDate = $.datepicker.parseDate($.datepicker.regional["it"].dateFormat, aData[_fnColumnIndex(index)]);
	          } catch (ex) {
	              return false;
	          }
	          if (dCellDate == null)
            ...
            $('#' + sFromId + ',#' + sToId, th).on('dp.change', function (e) {
                oTable.fnDraw();
                fnOnFiltered();
            })
            /*
        ...
        function fnCreateColumnSelect(oTable, aData, iColumn, nTh, sLabel) {
            ...
            var index = iColumn;
            var r = '<select class="search_init select_filter form-control"><option value="" class="search_init">' + sLabel + '</option>';
            var j = 0;
                ...
                if (aoColumn != null) {
                    ...
                    switch (aoColumn.type) {
                        ...
                        case "select":
                            ...
                            break;
                        case "label-number-range":
                            fnCreateLabelRangeInput(oTable);
                            break;
                        case "number-range":
                            fnCreateRangeInput(oTable);
                            break;
                        case "date-range":
                            fnCreateDateRangeInput(oTable,true);
                            break;
                        case "date":
                            fnCreateDateRangeInput(oTable,false);
                            break;
                        case "checkbox":
